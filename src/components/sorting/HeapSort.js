...
function heapify(i) {
    ...
    if (largest !== i) {
        ...
        timer = setTimeout(function () {
            ...
            $('line').eq(left - 1).attr("stroke", "#777");
            if (right < n) {
                $('line').eq(right - 1).attr("stroke", "#777");
            }
...
function swapMax(left, right) {
    if (p.y < v[n - 1].y) {
        ...
        $('.vlbl').eq(0).attr("y", p.y + 5);
        q.y -= 1;
        $('.vrtx').eq(n - 1).attr("cy", q.y);
        $('.vlbl').eq(n - 1).attr("y", q.y + 5);
    }
    else {
        if (left > -1 && p.x > v[n - 1].x) {
            ...
            q.x += 1;
            $('.vrtx').eq(n - 1).attr("cx", q.x);
            $('.vlbl').eq(n - 1).attr("x", q.x);
        }
...
function swap(i, largest, angle) {
    if (p.y < v[largest].y) {
        ...
        timer = setTimeout(function () {
            $('.vrtx').eq(i).attr("fill", "#eee");
            $('line').eq(i * 2).attr("stroke", "#777");
            if (i * 2 + 2 < n) {
                $('line').eq(i * 2 + 1).attr("stroke", "#777");
            }
            if (i < Math.floor(n / 2)) {
                heapify(largest);
            }
            // else {
            //     timer = setTimeout(maxHeap, delay / 2);
            // }
        }, delay / 2);
    }
}
